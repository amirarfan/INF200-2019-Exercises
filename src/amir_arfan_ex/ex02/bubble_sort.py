    ...
    for i in range(n_length):
        for j in range(n_length - i - 1):
            if sorted_data[j] > sorted_data[j + 1]:
                # Using the bubble sort method, checking if current iteration
                # is greater than the next iteration, if it is, they are swapped.
                # Possible to do this with while loop
